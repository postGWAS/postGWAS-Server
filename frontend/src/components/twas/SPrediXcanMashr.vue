...
<template>
    <div>
        ...
        <div class="inputCard">
            <h4>3.Choose data type.</h4>
            <el-radio-group v-model.number="flag" class="ml-4">
                <el-radio :label="0">GWAS summary statistics with hg38 position ID. <el-tooltip class="item" effect="dark"
                        content="The variant id must be the hg38 version. It may take more than ten minutes to run, please be patient."
                        placement="top-start">
                        <el-icon color="#1296DB" :size="20">
                            <Compass />
                        </el-icon>
                    </el-tooltip></el-radio>
                <el-radio :label="1">GWAS summary statistics with rs ID.</el-radio>
            </el-radio-group>
        </div>

        <div class="inputCard">
            <h4>4.The zscore column name in the input GWAS file.</h4>
            <el-input class="input" v-model="zscore" placeholder="Please input" />
        </div>

        <div class="inputCard">
            <h4>5.
                <span v-if="flag === 0">The hg38 position id column name in the input GWAS file.</span>
                <span v-if="flag === 1">The rs_id column name in the input GWAS file.</span>
            </h4>
            <el-input class="input" v-model="rsid" placeholder="Please input" />
        </div>

        <div class="inputCard">
            <h4>6.The effect allele name in the input GWAS file</h4>
            <el-input class="input" v-model="a1" placeholder="Please input" />
        </div>

        <div class="inputCard">
            <h4>7.The non-effect allele name in the input GWAS file</h4>
    ...
    <!-- <div class="inputCard">
        ...
        <div class="inputCard" v-show="flag === 1">
            <h4>8.The chromosome column name in the input GWAS file.</h4>
            <el-input class="input" v-model="chr" placeholder="Please input" />
        </div>

        <div class="inputCard" v-show="flag === 1">
            <h4>9.The position column name in the input GWAS file.</h4>
            <el-input class="input" v-model="pos" placeholder="Please input" />
        </div>

        <br>
        <el-button class="button" type="primary" round @click="runSPrediXcanMashr(); func.clickHandler($event)">RUN
...
const runSPrediXcanMashr = () => {
    if (dataCheck()) {
        const data = {
            ...
            // model: model.value,
            flag: flag.value,
            chr: flag.value === 1 ? chr.value : null,
            pos: flag.value === 1 ? pos.value : null,
            email: emailStore.email,
        }
        twasAPI.runSPrediXcanMashr(data).then((res) => {
...
const dataCheck = () => {
    ...
    // if (check.isInputIllegal(model)) {
    //     ElMessage.error('Please choose model');
    //     return false;
    // }

    if (check.isInputIllegal(flag) && flag.value !== 0) {
        ElMessage.error('Please input flag');
        return false;
    }

    if (flag.value !== 1 && flag.value !== 0) {
        ElMessage.error('Illegal flag');
        return false;
    }
