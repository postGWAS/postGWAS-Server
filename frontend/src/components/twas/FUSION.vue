...
<template>
    <div>
        ...
        <div class="inputCard">
            <h4>8.The gene expression model to run.
                <el-tooltip class="item" effect="dark" placement="right-start">
                    <template #content>
                        <div style="font-size: small; margin-left: 2%;">
                        <!-- <p><strong>blup</strong>: Best Linear Unbiased Predictor computed from all SNPs. Requires GEMMA
                                                        in path.</p> -->
                        <!-- <p><strong>bslmm</strong>: Bayesian Sparse Linear Model (spike/slab MCMC). Requires GEMMA in
                                                        path.</p> -->
                            <p><strong>enet</strong>: Elastic-net regression (with mixing parameter of 0.5).</p>
                            <p><strong>lasso</strong>: LASSO regression. Requires PLINK in path</p>
                            <p><strong>top1</strong>: Single best eQTL.</p>
                        </div>
                    </template>
                    <el-icon color="#1296DB" :size="20">
                        <Compass />
            ...
            <el-select v-model="model" class="m-2" placeholder="expression model" size="large" filterable clearable>
                <el-option v-for="item in modelOptions" :key="item.label" :label="item.label" :value="item.value" />
            </el-select>
        </div>

        <div class="inputCard">
            <h4>9.Set the threshold of pvalue.
...
const model = ref();
const modelOptions = [
    // { label: "blup", value: "blup" },
    // { label: "bslmm", value: "bslmm" },
